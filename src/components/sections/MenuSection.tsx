...
function MenuCard({ name, desc, price, img }: { name: string; desc: string; price: string; img: string }) {
  return (
    <div className="group rounded-xl overflow-hidden border transition-all duration-300" style={{ backgroundColor: "#201f1f", borderColor: "rgba(255,255,255,0.05)" }}>
      <div className="overflow-hidden" style={{ height: "192px" }}>
        <img src={img} alt={name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
      </div>
      <div className="p-6 flex justify-between items-end">
        <div>
          <h4 className="mb-2" style={{ fontFamily: "Be Vietnam Pro, sans-serif", fontSize: "18px", fontWeight: 700, color: "#e5e2e1" }}>{name}</h4>
          <p style={{ fontFamily: "Be Vietnam Pro, sans-serif", fontSize: "14px", color: "#e6bdbb" }}>{desc}</p>
        </div>
        <div className="shrink-0 ml-4 px-3 py-1 rounded border" style={{ fontFamily: "Space Grotesk, sans-serif", fontSize: "12px", fontWeight: 700, letterSpacing: "0.1em", color: "#ffba20", backgroundColor: "#131313", borderColor: "rgba(255,219,157,0.2)" }}>{price}</div>
      </div>
    </div>
  );
}
...
export default function MenuSection() {
  return (
    <section id="menu" style={{ scrollMarginTop: "72px" }}>
      ...
      <div className="py-16 flex items-center justify-center text-center" style={{ backgroundColor: "#0e0e0e", padding: "4rem 1.25rem" }}>
        <div>
